...
class LocationService {
  //location is laibrary in flutter
  ...
  Future<geo.Placemark?> getAddressInfo(LocationData locationData,
    ...
    if (placemarks.isNotEmpty) {
      //must choose [0] becouse the best 
      return placemarks[0];
    } else {
      return null;
    }
...
//for two permission user (two function)
  Future<bool> isLocationEnabeld() async {
    bool _serviceEnabled;
    _serviceEnabled = await location.serviceEnabled();
    if (!_serviceEnabled) {
      //here request service from user to open location
      _serviceEnabled = await location.requestService();
      if (!_serviceEnabled) {
        //!-- Message to show -- to know user is applacation do not user without loacation
        return false;
      }
    }
